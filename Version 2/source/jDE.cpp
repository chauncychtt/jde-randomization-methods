...
void jDE::update(uint dist_id){
  ...
  for( uint i = 0; i < size; i++ ){
  	...
  	if(dist_id == 1) {
    	...
    	r1 = fmod(fabs(gsl_ran_gaussian(r, 1.0)), 1.0);
	    ...
	    r3 = fmod(fabs(gsl_ran_cauchy(r, 1.0)), 1.0);
	    r4 = fmod(fabs(gsl_ran_cauchy(r, 1.0)), 1.0);
    } else if(dist_id == 4) {
    	r1 = logisticMap(random(rng), 4.0);
	    r2 = logisticMap(random(rng), 4.0);
	    r3 = logisticMap(random(rng), 4.0);
	    r4 = logisticMap(random(rng), 4.0);
    } else if(dist_id == 5) {
    	r1 = kentMap(random(rng), 0.7);
	    r2 = kentMap(random(rng), 0.7);
	    r3 = kentMap(random(rng), 0.7);
	    r4 = kentMap(random(rng), 0.7);
    }
...
void jDE::runDE(uint ndim, uint ps, const vDouble& genes, vDouble& n_gen, const double x_min, const double x_max, uint dist_id){
  assert( ndim > 0 );
  ...
  for( ; i < ps; i++ ){
  	...
  	if(dist_id == 1) {
    	...
    	/* Get three mutually different indexes */
	    do n1 = (int)remap(kentMap(random(rng), 0.7), 0.0, 1.0, 0.0, ps-1); while (n1 == i);
	    do n2 = (int)remap(kentMap(random(rng), 0.7), 0.0, 1.0, 0.0, ps-1); while (n2 == i || n2 == n1);
	    do n3 = (int)remap(kentMap(random(rng), 0.7), 0.0, 1.0, 0.0, ps-1); while (n3 == i || n3 == n1 || n3 == n2);
    }
